...
- 識別子・コミットメッセージ・ブランチ名: **英語**
- docstring・コード内コメント・ユーザー向けドキュメント(README、`docs/source/` の解説ページ): **日本語**
...
- ルール抑制ポリシー: まずコードの修正を検討してください。どうしても抑制する場合は、行単位なら理由コメント付きの `# noqa: <RULE>` / `# ty: ignore[<rule>]`、ファイル横断なら `pyproject.toml` の `per-file-ignores` に**理由コメント付き**で追加します(既存エントリの書式に合わせること)。
...
- Google スタイル(Napoleon)で記述します。要約行 1 行 + 空行 + セクションの構成です。
- 本文は日本語で書き、要約行は「〜する。」の形で終えます。セクション見出し(`Args:` / `Returns:` / `Raises:` / `Examples:`)は英語の Google スタイルのままです。
...
- 現時点の要件を満たす最小の構造で書きます。将来の拡張を見越した基底クラス・インターフェース・プラグイン機構・レジストリ・設定の外部化・ラッパー層・汎用ユーティリティは、**具体的な利用者が 2 つ以上現れるまで**導入しません。
- 関数で済むものをクラスにしないでください。標準ライブラリで足りるなら依存パッケージを追加しないでください。
- 型についても同様です。具体型で書き、抽象化は上記「型ヒント」の条件を満たす場合に限ります。
